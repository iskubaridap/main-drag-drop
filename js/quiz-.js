...
if ((window.location.href).search('/pages') > 0 && document.body.getAttribute('data-page-type') == 'quiz') {
    ...
    if (dragDrop) {
        ...
        const submitBtnWrap2 = document.createElement('div');
        const quizSubmitBtn2 = document.createElement('button');
        const quizTryAgainBtn2 = document.createElement('button');
        const quizDefendBtn2 = document.createElement('button');
        ...
        dragdropSeriesCorrectAns = Array.from(document.querySelectorAll('.drag-elem-wrap')).map(val => {
            var _a;
            const elem = val;
            return { attrVal: (elem.querySelector('.draggable').getAttribute('data-draggable')), ans: (elem.querySelector('.draggable').getAttribute('data-ans')), dragTxt: elem.querySelector('p').innerText };
        });
        ...
        submitBtnWrap2.id = 'submit-btn-wrap-2';
        quizSubmitBtn2.id = 'quiz-submit-2'
        quizTryAgainBtn2.id = 'quiz-try-again-2'
        quizDefendBtn2.id = 'quiz-defend-2'
        dropdownUI.classList.add('d-sm-block', 'd-md-block', 'd-lg-none');
        quizSubmitBtn2.classList.add('btn', 'yellow', 'submit-btns', 'active');
        quizTryAgainBtn2.classList.add('btn', 'yellow', 'submit-btns');
        quizDefendBtn2.classList.add('btn', 'yellow', 'submit-btns');
        quizSubmitBtn2.type = 'button';
        quizTryAgainBtn2.type = 'button';
        quizDefendBtn2.type = 'button';
        quizSubmitBtn2.innerText = 'Submit';
        quizTryAgainBtn2.innerText = 'Try Again';
        quizDefendBtn2.innerText = 'Next';
        tabIndex = 1;
        document.querySelectorAll('.droppable-item-wrap').forEach((val, index) => {
            var _a;
            const elem = val;
        ...
        submitBtnWrap2.appendChild(quizSubmitBtn2);
        submitBtnWrap2.appendChild(quizTryAgainBtn2);
        submitBtnWrap2.appendChild(quizDefendBtn2);
        dropdownUI.appendChild(submitBtnWrap2);
